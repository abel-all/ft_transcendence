...
function ProfileSecurity(className) {
  ...
  useEffect(() => {
    NewPassword != ConfirmPassword
      ? HandelSetingErrors(CheckConfirm)
      : CleanErros(CheckConfirm)
  }, [NewPassword, ConfirmPassword])

  useEffect(() => {
    !passwordValidRegux.test(NewPassword)
      ? HandelSetingErrors(ValidMessage)
      : CleanErros(ValidMessage)
  }, [NewPassword])
  ...
  const HandelChangingPassword = (e) => {
    ...
    if (Errors.length != 0) setShowErrors(true)
      ...
      axios
        .post('https://www.fttran.tech/api/auth/passwordchange/', {
        ...
        .then((res) => {
          ...
          } else console.log('Done Changing The password !', res)
        })
        .catch((err) => {
  ...
  return (
    <div
      className={
        'h-[1093px]' + (className.className ? ` ${className.className}` : '')
      }
    >
      ...
      <MenuBar
        className="md:hidden SecuritySettings flex flex-row justify-between font-[500] font-[Outfit] py-[35px] "
      ...
      <div className="linksHolder p-[17px]">
        ...
        <div
          ...
          className="DeleteSettingsButton text-[16px] font-[500] font-[Outfit] mb-[20px] cursor-pointer text-[#B63C3C] underline"
        >
          {' '}
          Delete Account
        </div>
          ...
          <form action="" method="" className="relative">
            <div className="m-[auto] py-[40px] relative px-[40px] mb-[200px] w-full max-w-[460px] border border-[#626262] rounded-[7px] bg-gradient-to-b from-[#152c2a] to-[#16181c] via-[#161c20] mt-[120px] max-sm:border-none max-sm:px-[0px] max-sm:bg-gradient-to-b max-sm:from-transparent max-sm:to-transparent max-sm:mt-[20px] max-sm:mb-[0px]">
              <ul className="ShowErrors text-[red] list-disc list-inside">
                {ShowErrors &&
                  ...
                  Errors.map((error, index) => {
                    if (error != PinError) return <li key={index}>{error}</li>
                  })}
              </ul>
              <img
                className="w-[32px] h-[32px] absolute top-[5px] right-[5px] cursor-pointer"
                onClick={DisplayHandler}
                src={close}
                alt=""
              />
              ...
              <div onClick={HandelChangingPassword} className="text-black">
                <Button title="Change My Password" type="submit" />
              </div>
            </div>
          </form>
        )}
              ...
              <div className="flex flex-col my-[20px] justify-around items-center border-0 bg-transparent w-full h-[58px]">
                ...
                <p className="py-[10px] text-center">
                  {!DeleteConfirmed ? (
                    ...
                    <input
                      className="text-black outline-none p-[5px] text-center rounded-sm"
                      name="confirm"
                      type="number"
                ...
                <div
                  onClick={(e) => {
                    if (!DeleteConfirmed) {
                      ...
                      axios
                        .get(
                          'https://www.fttran.tech/api/auth/delete-account/send-code'
                        )
                        .then((res) => {
                      ...
                      axios
                        .post(
                          'https://www.fttran.tech/api/auth/delete-account/check-code',
                          {
                            code: SixDigit,
                          }
                        )
