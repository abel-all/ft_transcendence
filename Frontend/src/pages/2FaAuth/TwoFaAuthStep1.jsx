...
const TwoFaAuthStep1 = () => {
    ...
    const verifyOtpCode = async () => {
        ...
        if (/^[0-9]{6,6}$/.test(code)) {
            await Axios.post("https://www.fttran.tech/api/auth/2fa/verify/device/", {
                otp_code: code,
            },
            {
    ...
    const handleEnableButtonClick = async () => {
        await Axios.get("https://www.fttran.tech/api/auth/2fa/enable/",
        {
            withCredentials:true,
        })
        .then(response => {
    ...
    if (isEnable)
        return (
            <div className="w-full h-[100vh] flex justify-center items-center">
                <button className="main-color-gradient p-[6px] w-full max-w-[195px] rounded-[5px] text-center text-[#1cc]" onClick={handleEnableButtonClick}>
                    Enable 2FA
                </button>
            </div>
        )
            ...
            <div className="input-gradient px-[60px] w-full max-w-[500px] h-[600px] bg-[#7b9d18] rounded-[15px] flex flex-col justify-between">
                <div className="text-input-container flex flex-col gap-[7px]">
                    ...
                    <div className="text-[#eee] opacity-50 text-[14px]">
                        Step 1: Visit the App Store to get an authenticator app like Google Authenticator or Authy, then follow the app's instructions to set up an account with them.<br />Step 2: Use your authenticator app to scan the barcode below or get a token to enter manually instead.
                    </div>
