...
function Messages({setMessages, username, className}) {
    ...
    useEffect(() => {
        async function fetchUsername() {
            await axios.get('http://10.12.9.14:8000/api/WhoAmI/')
            .then(res => {
    ...
    useEffect(() => {
        const handleScroll = async () => {
            if (messagesRef.current.scrollTop === 0 && index.current < Arr.length) {
                ...
                try {
                    const res = await axios.get(`http://10.12.9.14:8000/messages/${Arr[index.current]}`, { withCredentials: true });
                    index.current++;
                    ...
                    setTimeout(() => {
                        ...
                        setIsLoading(false);
                        console.log(Arr[index.current], " fetched and scroll went to ", (newScrollHeight - oldScrollHeight), newScrollHeight, oldScrollHeight);
                    }, 0);
