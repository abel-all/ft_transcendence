...
const FriendListHost = ({ className, UrlUsername }) => {
    ...
    const fetchmydata = async () => {
      try {
        const res = await axios.get(
          `https://www.fttran.tech/api/profile/friends/`,
        ...
        console.log(
          `List of FRIENDS fetchig data Error for user ${UrlUsername}`
        )
      }
    }
    fetchmydata()
  ...
  return (
    <>
      <div
        className={
          'relative w-[620px] p-[5px] md:p-[25px] bg-[var(--bg-color)] border-[1px] border-[#626262]' +
          (className ? ` ${className}` : '')
        }
      >
        ...
        <button>
          <img
            className=" top-[20px] right-[18px] absolute w-[25.6px] h-[25.6px]"
            src={userIcon}
            alt=""
          />
        </button>
