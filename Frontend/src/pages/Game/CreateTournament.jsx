...
const TournamentCard = ({image, bgColor, iconColor, title, description, hoverColor}) => {
    ...
    return (
        <div onClick={clickHandler} className={`select-none cursor-pointer w-[350px] h-[65px] bg-[#2C3134] group ${hoverColor} duration-500 rounded-[15px] flex gap-[18px] items-center pl-[18px]`}>
            <div className={`flex justify-center items-center w-[35px] h-[35px] ${bgColor} ${iconColor} duration-500 rounded-full`}>
                <img className="w-[25px] h-[25px]" src={image} />
            </div>
            <div className="flex flex-col">
                <div className="text-[#fff6f9] text-[18px] font-medium">
                    {title}
                </div>
